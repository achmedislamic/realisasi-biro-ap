<div class="flex flex-col gap-y-4">
    ...
    <x-table.index :model="$kegiatans">
        ...
        <x-slot name="table_actions">
            @if ($programId != 0)
            <x-button primary :href="route('kegiatan.form', $programId)" label="Tambah" />
            @endif
        </x-slot>
        ...
        <tbody>
            ...
            <x-table.tr>
                ...
                <x-table.td class="hover:underline hover:cursor-pointer hover:text-yellow-500" wire:click="pilihIdKegiatanEvent({{ $kegiatan->id }}, '{{ $menu }}', '{{ $opdId }}', '{{ $subOpdId }}')">
                    {{ $kegiatan->nama }}
                ...
                <x-table.td>
                    ...
                    <x-button.circle warning xs icon="pencil"
                            :href="route('kegiatan.form', [$programId, $kegiatan->id])" />
