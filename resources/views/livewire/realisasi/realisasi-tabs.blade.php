...
<div x-data="{ pemberitahuan: true }" class="pb-12">
                    ...
                    <div class="pt-3 w-1/2 flex gap-2">
                        ...
                        @if (auth()->user()->isAdmin())
                            <div class="w-full flex flex-row items-end space-x-3">
                                <div class="w-full">
                                    <x-native-select label="Bidang" placeholder="Semua Bidang"
                        ...
                        @if (auth()->user()->isAdmin())
                            <div class="w-full flex flex-row items-end space-x-3">
                                <div class="w-full">
                                    <x-native-select label="Sub OPD" placeholder="Semua Sub OPD (Unit)"
                    ...
                    <ul class="flex flex-wrap -mb-px text-sm font-medium text-center">
                        ...
                        <li class="mr-2" role="presentation">
                            <button @click="tab = 'subKegiatan'"
                                :class="tab == 'subKegiatan' ?
                                    'text-yellow-600 font-bold border-transparent hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300' :
                                    ''"
                                class="inline-block p-4 border-b-2 rounded-t-lg" id="sub-kegiatan-tab"
                                data-tabs-target="#subKegiatan" type="button" role="tab" aria-selected="false">Sub
                                Kegiatan</button>
                        </li>

                        <li class="mr-2" role="presentation">
                            <button @click="tab = 'subRincianObjek'"
                                :class="tab == 'subRincianObjek' ?
                                    'text-yellow-600 font-bold border-transparent hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300' :
                                    ''"
                        ...
                        <li class="mr-2" role="presentation">
                            <button @click="tab = 'objekRealisasi'"
                                :class="tab == 'objekRealisasi' ?
                                    'text-yellow-600 font-bold border-transparent hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300' :
                                    ''"
                                class="inline-block p-4 border-b-2 rounded-t-lg" id="objek-realisasi-tab"
                                data-tabs-target="#objek-realisasi" type="button" role="tab"
                                aria-controls="profile" aria-selected="false">Rincian Belanja</button>
                        </li>
    ...
    @if (filled($jadwal))
        <div x-show="pemberitahuan" x-transition
            class="fixed bottom-0 left-0 z-20 w-full p-2 bg-white border-t border-gray-200 shadow md:flex md:items-center md:justify-between md:p-4 dark:bg-gray-800 dark:border-gray-600">
            <span class="text-sm text-gray-500 sm:text-center dark:text-gray-400">
                Jadwal input realisasi untuk bulan {{ $jadwal->bulan->translatedFormat('M Y') }} adalah hingga <span
                    class="text-red-400">{{ \App\Helpers\FormatHelper::tanggal($jadwal->tanggal_waktu->timezone('Asia/Makassar'), true) }}</span>
            </span>
